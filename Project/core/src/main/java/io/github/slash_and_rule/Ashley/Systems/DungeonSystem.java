...
public class DungeonSystem extends EntitySystem {
    ...
    public DungeonSystem(int priority, DungeonManager dungeonManager, PhysicsBuilder physicsBuilder, OrthographicCamera camera, float scale) {
        super(priority);
    ...
    private void scheduleRoom(int direction, DungeonRoom room) {
        dungeonManager.getData(room, roomData -> {
            dungeonBuilder.scheduledMakeRoom(schedule, roomData, neighbours, new DoorHandler(), entity -> {
    ...
    public void init(LoadingScreen loader) {
        ...
        dungeonManager.getData(myRoom, loader, roomdata -> {
            ...
            System.out.println(Arrays.toString(Mappers.dungeonMapper.get(room).doors));
        });
