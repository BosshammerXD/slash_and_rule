...
public class DungeonSystem extends EntitySystem {
    ...
    private QuadData<RoomEntity> neighbours = new QuadData<>();
    ...
    private void addRoomEntity(RoomEntity roomEntity, int direction) {
        ...
        if (direction < 0) {
            ...
            neighbours.set(direction, roomEntity);
        } else {
            return;
        }
    ...
    private void removeRoomEntity(int direction) {
        ...
        if (direction < 0 && room != null) {
            roomEntity = room;
            room = null;
        } else if (direction < 4 && neighbours.get(direction) != null) {
            roomEntity = neighbours.get(direction);
            neighbours.clear(direction);
        }
        if (roomEntity == null) {
            return;
        }
    ...
    private void change_room(int direction) {
        ...
        // neighbour
        if (neighbours.isSet((direction + 2) % 4))
            removeRoomEntity((direction + 2) % 4);
        neighbours.set((direction + 2) % 4, room);
        room = neighbours.get(direction);
        neighbours.clear(direction);
        ...
        scheduleRoom(direction, dungeonRoom.neighbours.get(direction));
        scheduleRoom((direction + 1) % 4, dungeonRoom.neighbours.get((direction + 1) % 4));
        scheduleRoom((direction + 3) % 4, dungeonRoom.neighbours.get((direction + 3) % 4));
    ...
    public void init(LoadingScreen loader) {
        ...
        DungeonRoom other = myRoom.neighbours.down;
        ...
        dungeonManager.getData(other, loader, roomdata2 -> {
            neighbours.set(1, dungeonBuilder.makeRoom(roomdata2, other.neighbours, new DoorHandler()));
            addRoomEntity(neighbours.down, 1);
